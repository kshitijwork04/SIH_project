-- ============================================================================
-- Kabadiwala Connect — ONE-SHOT FULL SETUP (Level 1 + Level 2, schema + seed)
-- Paste this ENTIRE file into your Supabase project's SQL editor and press Run.
-- It is idempotent: safe to run on a fresh project OR on one that already has
-- the Level 1 tables. Creates all tables/columns, RLS policies, and sample
-- data needed for the whole app (7 categories, real trends, ranked recyclers,
-- GPS traceability, collector profiles).
-- ============================================================================
...
-- Level 2: handover traceability (journey: created -> matched -> handed_over -> confirmed)
...
-- Level 2: collector minimal profile (NO name / ID / address by design)
...
-- ============================================================================
-- 2) LEVEL 2 COLUMNS (idempotent adds)
-- ============================================================================

-- materials: extra create-a-lot fields
...
-- prices: replace flat 4-row shape with a full historical dataset.
-- The old column is dropped (harmless) and the new ones added.
alter table prices
...
-- recyclers: full authorized dataset
...
-- transactions: payment status + final price
...
-- ============================================================================
-- 3) ROW LEVEL SECURITY (demo-friendly: anon can read + write)
-- ============================================================================
...
drop policy if exists "anon all materials" on materials;
drop policy if exists "anon all prices" on prices;
drop policy if exists "anon all recyclers" on recyclers;
drop policy if exists "anon all transactions" on transactions;
drop policy if exists "anon all traceability" on traceability;
drop policy if exists "anon all collectors" on collectors;
...
-- Recyclers (5 full authorized profiles)
...
-- Prices: 7 categories x ~5 historical rows (drives the price board + trends)
insert into prices (
...
-- ============================================================================
-- 5) OPTIONAL: public storage bucket for lot photos
-- (Not required — the app falls back to inline photos if the bucket is missing.)
-- ============================================================================
